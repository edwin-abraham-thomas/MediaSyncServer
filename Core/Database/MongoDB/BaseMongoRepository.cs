...
public abstract class BaseMongoRepository<TDocument> : IBaseMongoRepository<TDocument> where TDocument : IMongoDocument
{
    ...
    protected BaseMongoRepository(IMongoConfig mongoConfig)
    {
        _mongoClient = new MongoClient(mongoConfig.MongoConnectionString);
        _mongoDb = _mongoClient.GetDatabase(mongoConfig.MongoDatabaseName);
